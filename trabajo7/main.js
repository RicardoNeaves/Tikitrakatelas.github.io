...
async function searchPokemonById(id) {
    ...
    //Arreglo con los nombres de los pokemons
    pokemonNames.push(data.name)
    pokemonsNames = pokemonNames.sort(() => Math.random() - 0.5)
    ...
    pokemonSearched.forEach(pokemon => {
        ...
        draggableElements.innerHTML += `
            <div class="pokemon">
                <img id="${pokemon.name}" draggable="true" class="image" src="${pokemon.sprites.front_default}">
            </div>`;
    })
